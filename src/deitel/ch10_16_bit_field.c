/* Concept: structures */
/* Source: deitel:ch10_16 */
/* Example using a bit field */
#include <stdio.h>

struct example {
    unsigned a : 4;
    unsigned b : 4; /* Packed into the same storage unit as `a` */
};

struct example2 {
    unsigned a : 4;
    unsigned b : 4;
    int c;           /* Starts a new storage unit (aligned for `int`) */
    unsigned d : 4;  /* Starts a new storage unit */
};
